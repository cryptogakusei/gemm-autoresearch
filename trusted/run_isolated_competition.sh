...
readonly CONTAINER_TIMEOUT=40m
...
CONTAINER_ID="$("${DOCKER[@]}" create \
    ...
    --entrypoint /bin/bash \
    "$CUDA_IMAGE" \
    /competition/trusted/run_competition.sh \
    /candidate/candidate_gemm.cu \
    /results)"

set +e
timeout --signal=TERM --kill-after=10s "$CONTAINER_TIMEOUT" \
    "${DOCKER[@]}" start --attach "$CONTAINER_ID"
container_status=$?
set -e
if (( container_status == 124 )); then
    echo "sandbox exceeded the $CONTAINER_TIMEOUT wall-clock limit" >&2
    "${DOCKER[@]}" rm --force "$CONTAINER_ID" >/dev/null 2>&1 || true
    CONTAINER_ID=""
    exit 124
fi
